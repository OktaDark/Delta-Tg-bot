...
def main_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    ...
    builder.row(KeyboardButton(text=t(user_id, "btn_feedback")))
...
def help_inline_keyboard(user_id: int) -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(text=t(user_id, "btn_dev"), callback_data="contact_dev")
    )
    return builder.as_markup()
...
def admin_keyboard() -> InlineKeyboardMarkup:
    ...
    builder.row(
        InlineKeyboardButton(
            text="💾 Резервна копія JSON", callback_data="adm_backup_json"
        ),
        InlineKeyboardButton(
            text="📋 Резервна копія CSV", callback_data="adm_backup_csv"
        ),
    )
    builder.row(
...
async def cmd_help_command(message: types.Message):
    ...
    track_usage(uid)
    await message.answer(
        t(uid, "help_text"), parse_mode="HTML", reply_markup=help_inline_keyboard(uid)
    )
...
async def admin_callbacks(callback: types.CallbackQuery):
    ...
    # ── Загальна статистика ──────────────────────────────────────────────────
        ...
        for p in user_profiles.values():
            lang_counts[p.get("lang", "uk")] = (
                lang_counts.get(p.get("lang", "uk"), 0) + 1
            )
    ...
    # ── Резервна копія CSV (список користувачів) ─────────────────────────────
        ...
        writer.writerow(
            ["ID", "Ім'я", "Username", "Мова", "Дата входу", "Запитів", "Розрахунків"]
        )
        for uid_u, profile in user_profiles.items():
            writer.writerow(
                [
...
async def main_router(message: types.Message, state: FSMContext):
    ...
    if message.web_app_data:
        ...
        try:
            ...
            await message.answer(
                t(uid, "webapp_result", data=formatted), parse_mode="HTML"
            )
    ...
    if current_state == BotState.waiting_for_feedback.state:
        if message.text and message.text.startswith("/"):
            ...
            await state.clear()
            await message.answer(
                t(uid, "feedback_done"), reply_markup=main_keyboard(uid)
            )
        return
    ...
    if text in [TRANSLATIONS[l]["btn_help"] for l in TRANSLATIONS]:
        ...
        await message.answer(
            t(uid, "stats_text", count=count, total=total), parse_mode="HTML"
        )
